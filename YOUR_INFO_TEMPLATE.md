# 📝 Your Information Template

Copy this template, fill it out, then use it to update your portfolio!

---

## Personal Information

**Full Name:** Sanchit Bishwakarma
**Username/Handle:** sanchitbishwakarma
**Tagline/Bio:** I make exciting projects and build products that serve millions of users

**Email:** sanchitbishwakarma91@gmail.com
**Location:** Your City, Country (optional)

---

## Social Media Links

**GitHub:** https://github.com/sanchitbishwakarma
**LinkedIn:** https://linkedin.com/in/sanchitbishwakarma
**Twitter:** https://twitter.com/sanchitbishwakarma
**Instagram:** https://instagram.com/sanchitbishwakarma
**Portfolio/Website:** https://yourwebsite.com

---

## Projects (Add 4-6 of your best projects)

### Project 1
- **Title:** 
- **Description:** 
- **Technologies:** (e.g., JavaScript, React, Node.js)
- **GitHub Link:** 
- **Live Demo:** (optional)
- **Icon:** (Font Awesome class, e.g., "fab fa-github")

### Project 2
- **Title:** 
- **Description:** 
- **Technologies:** 
- **GitHub Link:** 
- **Live Demo:** 
- **Icon:** 

### Project 3
- **Title:** 
- **Description:** 
- **Technologies:** 
- **GitHub Link:** 
- **Live Demo:** 
- **Icon:** 

### Project 4
- **Title:** 
- **Description:** 
- **Technologies:** 
- **GitHub Link:** 
- **Live Demo:** 
- **Icon:** 

---

## Skills (Rate yourself honestly!)

### Skill 1
- **Name:** JavaScript
- **Level:** Expert / Advanced / Intermediate / Beginner
- **Percentage:** 95% (how proficient you are)
- **Icon:** fab fa-js
- **Color:** #f0db4f

### Skill 2
- **Name:** 
- **Level:** 
- **Percentage:** 
- **Icon:** 
- **Color:** 

### Skill 3
- **Name:** 
- **Level:** 
- **Percentage:** 
- **Icon:** 
- **Color:** 

(Add more skills as needed)

---

## Platforms/Channels (YouTube, Blog, etc.)

### Platform 1
- **Name:** YouTube Channel
- **Description:** 
- **Subscribers/Followers:** 
- **Content Count:** (videos, posts, etc.)
- **Link:** 
- **Icon:** fab fa-youtube

### Platform 2
- **Name:** 
- **Description:** 
- **Subscribers/Followers:** 
- **Content Count:** 
- **Link:** 
- **Icon:** 

---

## Font Awesome Icons Reference

Common icons you can use:

**Programming Languages:**
- JavaScript: `fab fa-js`
- Python: `fab fa-python`
- Java: `fab fa-java`
- PHP: `fab fa-php`
- React: `fab fa-react`
- Vue: `fab fa-vuejs`
- Angular: `fab fa-angular`
- Node.js: `fab fa-node`

**Tools & Platforms:**
- GitHub: `fab fa-github`
- GitLab: `fab fa-gitlab`
- Docker: `fab fa-docker`
- AWS: `fab fa-aws`
- Linux: `fab fa-linux`
- Windows: `fab fa-windows`
- Apple: `fab fa-apple`

**Social Media:**
- Twitter: `fab fa-twitter`
- LinkedIn: `fab fa-linkedin`
- Instagram: `fab fa-instagram`
- YouTube: `fab fa-youtube`
- Facebook: `fab fa-facebook`
- Discord: `fab fa-discord`

**General:**
- Code: `fas fa-code`
- Globe/Website: `fas fa-globe`
- Email: `fas fa-envelope`
- Download: `fas fa-download`
- Star: `fas fa-star`
- Heart: `fas fa-heart`
- Rocket: `fas fa-rocket`
- Database: `fas fa-database`
- Server: `fas fa-server`
- Mobile: `fas fa-mobile`
- Desktop: `fas fa-desktop`
- Shopping Cart: `fas fa-shopping-cart`
- Robot: `fas fa-robot`
- Tasks: `fas fa-tasks`
- Blog: `fas fa-blog`

Find more at: https://fontawesome.com/icons

---

## Color Codes for Skills

**Popular Language Colors:**
- JavaScript: `#f0db4f`
- Python: `#3776ab`
- Java: `#007396`
- HTML: `#e34c26`
- CSS: `#1572b6`
- React: `#61dafb`
- Vue: `#42b883`
- Node.js: `#68a063`
- PHP: `#777bb4`
- Ruby: `#cc342d`
- Go: `#00add8`
- Rust: `#000000`
- TypeScript: `#3178c6`
- C++: `#00599c`
- C#: `#239120`

**Tools:**
- Git: `#f05032`
- Docker: `#0db7ed`
- MongoDB: `#4db33d`
- PostgreSQL: `#336791`
- MySQL: `#4479a1`
- Redis: `#dc382d`
- Firebase: `#ffca28`

---

## Next Steps

1. Fill out this template with your information
2. Open `js/main.js` and update the `portfolioData` object
3. Open `index.html` and update personal info and social links
4. Add your profile picture to `images/profile.jpg` (or keep the SVG placeholder)
5. Test your site!

**Pro Tip:** Keep this file updated as you add new projects and skills!
